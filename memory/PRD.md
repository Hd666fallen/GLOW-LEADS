...
- **Iteration 6 (Feb 2026) — Per-finger customizer polish:**
  - **Live SVG hand visualization** above the finger row: 5 stylized fingers with nail tips that fill with the current per-finger colour and clip a thumbnail of the selected design. Tap any nail on the SVG to edit that finger. Active nail glows pink + gold. Right-hand view mirrors via `scaleX(-1)`. Smooth `transition: all 0.2s ease`.
  - Reworked finger thumbnails into a dedicated `<NailPreview/>` component that **only ever renders the DESIGN image** (with `object-position: 50% 35%` for proper nail-area framing) plus the colour overlay — never the customer's uploaded hand photo.
