...
export default function CustomerFunnel() {
  ...
  return (
    <div className="funnel-theme" data-testid="customer-funnel">
      ...
      {/* Color sheet — updates fingerState colors too */}
        ...
        <SheetContent side="bottom" className="h-[80vh] overflow-y-auto rounded-t-3xl">
          ...
          <ColorPalette
            ...
            onSelect={(c) => {
              setColor(c);
              setSheet(null);
              // Update ALL fingers to the new base color
              setFingerState(prev => {
                if (!prev) return prev;
                const next = {};
      ...
      {/* Design sheet — updates fingerState designs too */}
        ...
        <SheetContent side="bottom" className="h-[85vh] overflow-y-auto rounded-t-3xl">
          ...
          <DesignGrid
            ...
            onPick={(d) => {
              setSheet(null);
              setDesign(d);
              // Update ALL fingers to the new base design
              setFingerState(prev => {
                if (!prev) return prev;
                const next = {};
                Object.keys(prev).forEach(fid => {
      ...
      {/* Shape sheet — updates fingerState shapes too */}
        ...
        <SheetContent side="bottom" className="h-[80vh] overflow-y-auto rounded-t-3xl">
          ...
          <ShapeGrid
            ...
            onPick={(s) => {
              setSheet(null);
              setShape(s);
              // Update ALL fingers to the new base shape
              setFingerState(prev => {
                if (!prev) return prev;
                const next = {};
                Object.keys(prev).forEach(fid => {
      ...
      {shapes.map((s) => (
        ...
        >
          ...
          <div className="h-[65%] overflow-hidden">
            <img
              src={SHAPE_PHOTOS[s.id?.toLowerCase()] || SHAPE_PHOTOS[s.label?.toLowerCase()] || s.image}
              alt={s.label}
